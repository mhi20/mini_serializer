...
module MiniSerializer
    class Serializer
      ...
      def get_params_included_to_json(with_except: false)
        ...
        return { include: wams_association_for_to_json, except: wams_except_params_main_object } if with_except
      ...
      def json_serializer(object = nil)
        included_data = object || includes_to_serializer
      ...
      def includes_to_serializer
        ...
        data_with_includes = wams_main_object.includes(wams_association_object) if wams_has_many.any?
      ...
      def fetch_association_objects
        wams_has_many.each do |object_asoc, except_params|
        ...
        wams_has_one.each do |object_asoc, except_params|
          wams_association_for_to_json[object_asoc] = { except: except_params } # for to_json hash
          wams_association_object << object_asoc # for included (relation record)
        end
      end
